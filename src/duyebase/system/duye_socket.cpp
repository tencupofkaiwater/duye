...
int64 Transfer::send(const int32 sockfd, const int8* data, const uint64 len)
{
    return ::send(sockfd, data, len, MSG_NOSIGNAL);
}

int64 Transfer::sendmsg(const int32 sockfd, const struct msghdr* msg)
{
    return ::sendmsg(sockfd, msg, MSG_NOSIGNAL);
}

int64 Transfer::sendto(const int32 sockfd, IPv4Addr& dst_addr, const int8* data, const uint64 len)
{
    return ::sendto(sockfd, data, len, MSG_NOSIGNAL, (const struct sockaddr*)&dst_addr.addr(), dst_addr.addrLen());	
}
...
int64 Transfer::recvmsg(const int32 sockfd, struct msghdr* msg, const bool block)
{
    if (block)
    {
    ...
    {
        return ::recvmsg(sockfd, msg, MSG_DONTWAIT);
    }
}

int64 Transfer::recvfrom(const int32 sockfd, IPv4Addr& srcAddr, int8* buffer, const uint64 size, const bool block)
{
    uint32 addrLen = srcAddr.addrLen();
    if (block)
    {
        return ::recvfrom(sockfd, buffer, size, 0, (struct sockaddr*)&srcAddr.addr(), &addrLen);
    }
    else
    {
        return ::recvfrom(sockfd, buffer, size, MSG_DONTWAIT, (struct sockaddr*)&srcAddr.addr(), &addrLen);
    }
}
